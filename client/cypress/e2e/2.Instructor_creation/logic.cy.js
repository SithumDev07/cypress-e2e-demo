...
describe("Instructor Creation", () => {
  beforeEach(() => {
    ...
    cy.get('[href="/register/instructor"] > .MuiButtonBase-root').as(
  ...
  describe("First Name Validation", () => {
    it("should show required field error when the field is empty", () => {
      cy.contains("Submit").click();

      cy.contains("This field is required").should("exist");
    });

    it("should validate the first name entered", () => {
      // Enter an invalid name (e.g., a name containing numbers)
      cy.get('[name="firstName"]').type("Avishka123");
      cy.contains("Name is invalid").should("exist");

      // Enter a name with special characters
      cy.get('[name="firstName"]').clear().type("Avishka@Doe");
      cy.contains("Name is invalid").should("exist");

      // Enter a valid name
      cy.get('[name="firstName"]').clear().type("Avishka");

      // Check that the error messages disappear
      cy.contains("This field is required").should("not.exist");
      cy.contains("Name is invalid").should("not.exist");
      cy.contains("Name is too long").should("not.exist");
    });
  });
  describe("Last Name Validation", () => {
    ...
    it("should validate the last name entered", () => {
      // Enter an invalid name (e.g., a name containing numbers)
      cy.get('[name="lastName"]').type("Rodrigo123");
      cy.contains("Name is invalid").should("exist");

      // Enter a name with special characters
      cy.get('[name="lastName"]').clear().type("Rodrigo@Doe");
      cy.contains("Name is invalid").should("exist");

      // Enter a valid name
      cy.get('[name="lastName"]').clear().type("Rodrigo");
  ...
  describe("Select Subject", () => {
    it("should open and blur the Subject type dropdown", () => {
      ...
      cy.get('[placeholder="Select examination level"]').as(
        "levelAutocomplete"
      );
  ...
  describe("Password and Confirm Password Test", () => {
    it("should not submit the form if passwords do not match", () => {
      ...
      cy.url().should(
        "eq",
        (Cypress.config().href = "http://localhost:3000/register/instructor")
    ...
    it("should reject a weak password and clear the fields", () => {
      ...
      cy.wrap(weakPasswords).each((weakPassword) => {
        cy.get('[placeholder="Enter password"]')
          .as("password")
          .type(weakPassword);
        cy.get('[placeholder="Enter password again"]')
          .as("confirmPassword")
          .type(weakPassword);
